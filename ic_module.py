...
def a_func():
    FileNames = ["True.npy", "False.npy"]
    ClassNames = ["正常", "異常"]
    hw = {"height":32, "width":32}        # リストではなく辞書型 中かっこで囲む

    def syuturyoku_txt(txt_data):
        #dir = './'

        file_name = '/home/pi/pyparrot/kekka.txt'
    ...
    ############# 学習 #############
    ################################
    def Learning(tsnum=30, nb_epoch=20, batch_size=8, learn_schedule=0.9):
    ...
    def TestProcess(imgname):
        modelname_text = open("/home/pi/pyparrot/model.json").read()
        ...
        txt1 = imgname
        txt2 = ">> この画像は「" + textlist[np.argmax(pred)].replace(",", "") + "」です。"
        print(imgname)
        print(">> 計算結果↓\n" + str(pred))
    ...
    def test():
            files = glob.glob("/home/pi/output" + "/*.jpg")
            cn1 = 0; cn2 = 0;
